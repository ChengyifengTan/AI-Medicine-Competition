...
if task == 'positioning':
    ...
    # First split into training and validation sets, then perform oversampling
    x_train, x_val, y_train, y_val = train_test_split(x, y, test_size=0.2, random_state=42)
    ...
    # Convert training data to NumPy format for oversampling
    x_train_np = x_train.numpy()
    y_train_np = y_train.numpy()
    
    # Apply random oversampling (only to training set)
    ros = RandomOverSampler(random_state=42)
    
    # Reshape data to fit the oversampler
    ...
    # Convert reshaped data back to original shape
